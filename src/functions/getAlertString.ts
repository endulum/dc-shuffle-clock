...
export default function getAlertString(
  time: ITime,
  settings: IClockSettings
): string | null {
  if (
    // we're approaching an hourly and we need to be warned when or after it happens
    time.minutes === 0 &&
    ...
    if (
      !settings.noHourly &&
      time.seconds === Math.abs(settings.customHourlyDelay)
    ) {
      return `The site hourly happened ${Math.abs(
        settings.customHourlyDelay
      )} seconds ago.`;
    }
  } else if (
    // we're approaching an hourly and we need to be warned before it happens
    time.minutes === 59
  ) {
    const secsAtAlert =
      60 -
